...
export class CoreCourseModuleSummaryComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.displayOptions.displayGrades =
      this.displayOptions.displayGrades &&
      CoreCourseModuleDelegate.supportsFeature(
        this.module.modname,
        CoreConstants.FEATURE_GRADE_HAS_GRADE,
        true
      );

    this.displayOptions.displayDescription =
      this.displayOptions.displayDescription &&
      CoreCourseModuleDelegate.supportsFeature(
        this.module.modname,
        CoreConstants.FEATURE_SHOW_DESCRIPTION,
        true
      );
    ...
    if (this.component) {
      ...
      const debouncedUpdateSize = CoreUtils.debounce(async () => {
        ...
        const moduleSize =
          await CoreCourseModulePrefetchDelegate.getModuleStoredSize(
            this.module,
            this.courseId
          );
  ...
  protected async fetchContent(): Promise<void> {
    ...
    this.moduleNameTranslated = CoreCourse.translateModuleName(
      this.module.modname || ""
    );
  ...
  protected async loadModIcon(): Promise<void> {
    ...
    this.modicon = await CoreCourseModuleDelegate.getModuleIconSrc(
      this.module.modname,
      this.module.modicon,
      this.module
    );
  ...
  protected async getPackageStatus(refresh = false): Promise<void> {
    ...
    const moduleInfo = await CoreCourseHelper.getModulePrefetchInfo(
      this.module,
      this.courseId,
      refresh,
      this.component
    ...
    if (this.canPrefetch) {
      if (moduleInfo.downloadTime && moduleInfo.downloadTime > 0) {
        this.downloadTimeReadable = CoreTextUtils.ucFirst(
          moduleInfo.downloadTimeReadable
        );
      }
  ...
  async gotoBlog(): Promise<void> {
    ...
    await CoreNavigator.navigateToSitePath(
      AddonBlogMainMenuHandlerService.PAGE_NAME,
      { params }
    );
  }
  ...
  protected async fetchGrades(): Promise<void> {
    ...
    try {
      this.grades = await CoreGradesHelper.getModuleGrades(
        this.courseId,
        this.moduleId
      );
    } catch {
  ...
  async prefetch(): Promise<void> {
    ...
    try {
      ...
      const size = await CoreCourseModulePrefetchDelegate.getModuleDownloadSize(
        this.module,
        this.courseId,
        true
      );
  ...
  async removeFiles(): Promise<void> {
    ...
    if (this.prefetchLoading) {
      CoreDomUtils.showAlertTranslated(
        undefined,
