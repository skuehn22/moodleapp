...
export class CoreMainMenuMorePage implements OnInit, OnDestroy {
  ...
  loggedOut: any;
  constructor() {
    this.langObserver = CoreEvents.on(
      CoreEvents.LANGUAGE_CHANGED,
      this.loadCustomMenuItems.bind(this)
    );
    ...
    this.showScanQR =
      CoreUtils.canScanQR() &&
      !CoreSites.getCurrentSite()?.isFeatureDisabled(
        "CoreMainMenuDelegate_QrReader"
      );
  }
  ...
  ngOnInit(): void {
    // Load the handlers.
    this.subscription = CoreMainMenuDelegate.getHandlersObservable().subscribe(
      (handlers) => {
  ...
  initHandlers(): void {
    ...
    this.handlers = this.allHandlers.filter(
      (handler) => mainHandlers.indexOf(handler) == -1
    );
  ...
  openItem(item: CoreMainMenuCustomItem): void {
    CoreNavigator.navigateToSitePath("viewer/iframe", {
      params: { title: item.label, url: item.url },
    });
  }
  ...
  async scanQR(): Promise<void> {
    ...
    if (CoreCustomURLSchemes.isCustomURL(text)) {
      ...
      const treated = await CoreContentLinksHelper.handleLink(
        text,
        undefined,
        true,
        true
