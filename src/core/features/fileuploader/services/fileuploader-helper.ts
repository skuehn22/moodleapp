...
export class CoreFileUploaderHelperProvider {
  ...
  protected filePickerDeferred?: any;
  ...
  async chooseAndUploadFile(
    ...
    const result = await Chooser.getFileMetadata(
      mimetypes ? mimetypes.join(",") : undefined
    );
  ...
  async confirmUploadFile(
    ...
    if (!allowOffline && !CoreApp.isOnline()) {
      throw new CoreError(
        Translate.instant("core.fileuploader.errormustbeonlinetoupload")
      );
    }

    wifiThreshold =
      typeof wifiThreshold == "undefined"
        ? CoreFileUploaderProvider.WIFI_SIZE_WARNING
        : wifiThreshold;
    limitedThreshold =
      typeof limitedThreshold == "undefined"
        ? CoreFileUploaderProvider.LIMITED_SIZE_WARNING
        : limitedThreshold;

    if (size < 0) {
      return CoreDomUtils.showConfirm(
        Translate.instant("core.fileuploader.confirmuploadunknownsize")
      );
    } else if (
      size >= wifiThreshold ||
      (CoreApp.isNetworkAccessLimited() && size >= limitedThreshold)
    ) {
      const readableSize = CoreTextUtils.bytesToSize(size, 2);

      return CoreDomUtils.showConfirm(
        Translate.instant("core.fileuploader.confirmuploadfile", {
          size: readableSize,
        })
      );
  ...
  async copyAndUploadFile(
    ...
    try {
      ...
      const newName = await CoreFile.getUniqueNameInFolder(
        CoreFileProvider.TMPFOLDER,
        name
      );

      const filePath = CoreTextUtils.concatenatePaths(
        CoreFileProvider.TMPFOLDER,
        newName
      );
      ...
      fileEntry = await CoreFile.writeFileDataInFile(
        file,
        filePath,
        (progress: CoreFileProgressEvent) =>
  ...
  protected async copyToTmpFolder(
    ...
    const fileName =
      options?.fileName || CoreFile.getFileAndDirectoryFromPath(path).name;
    ...
    const destPath = CoreTextUtils.concatenatePaths(
      CoreFileProvider.TMPFOLDER,
      newName
    );
    if (shouldDelete) {
  ...
  protected createMaxBytesError(maxSize: number, fileName: string): CoreError {
    return new CoreError(
      Translate.instant("core.fileuploader.maxbytesfile", {
        $a: {
          file: fileName,
   ...
   */
  protected getChosenFileNameFromPath(
  ...
  async selectAndUploadFile(
    ...
    return <CoreWSUploadFileResult>(
      await this.selectFileWithPicker(maxSize, false, title, mimetypes, true)
    );
  }
  ...
  async selectFile(
    ...
    return <FileEntry>(
      await this.selectFileWithPicker(
        maxSize,
        allowOffline,
        title,
        mimetypes,
  ...
  protected async selectFileWithPicker(
        ...
        handler: async (): Promise<boolean> => {
          ...
          try {
            ...
            if (data.treated) {
              ...
              result = await this.uploadFileEntry(
                data.fileEntry,
                !!data.delete,
                maxSize,
                upload,
    ...
    this.actionSheet = await ActionSheetController.create({
      header: title
        ? title
        : Translate.instant(
            "core.fileuploader." + (upload ? "uploadafile" : "selectafile")
          ),
      buttons: buttons,
    });
   ...
   * @return Promise resolved when the file is uploaded.
   */
  async showConfirmAndUploadInSite(
    fileEntry: FileEntry,
    ...
    if (error) {
      if (error.code != 3) {
        ...
        const message = this.isNoAppError(error)
          ? Translate.instant("core.fileuploader.errornoapp")
   ...
   */
  protected treatImageError(
    error: number | string | CoreError | CoreCaptureError,
    defaultMessage: string
  ): CoreError {
    // Cancelled or error.
  ...
  async uploadAudioOrVideo(
    ...
    try {
      const medias = isAudio
        ? await CoreFileUploader.captureAudio(captureOptions)
  ...
  async uploadImage(
    ...
    if (fromAlbum) {
      const imageSupported =
        !mimetypes || CoreUtils.indexOfRegexp(mimetypes, /^image\//) > -1;
      const videoSupported =
        !mimetypes || CoreUtils.indexOfRegexp(mimetypes, /^video\//) > -1;
  ...
  async uploadFileEntry(
    ...
    const result = await this.uploadFileObject(
      file,
      maxSize,
      upload,
      allowOffline,
...
export const CoreFileUploaderHelper = makeSingleton(
