...
export class CoreFileUploaderProvider {
  ...
  clearTmpFiles(files: (CoreWSFile | FileEntry)[]): void {
    ...
    files.forEach((file) => {
      if (
          "remove" in file &&
          CoreFile.removeBasePath(file.toURL()).startsWith(
  ...
  getFilesToDelete(
    ...
    originalFiles.forEach((file) => {
      const stillInList = currentFiles.some(
          (currentFile) =>
              CoreFileHelper.getFileUrl(<CoreWSFile>currentFile) ==
  ...
  async getStoredFilesFromOfflineFilesObject(
    ...
    if (filesObject.offline > 0) {
      const offlineFiles = await CoreUtils.ignoreErrors(
    ...
    filetypes.forEach((filetype) => {
      ...
      if (filetype.indexOf("/") != -1) {
        ...
        typesInfo.push({
          ...
          extlist: CoreMimetypeUtils.getExtensions(filetype)
              .map(this.addDot)
              .join(" "),
        });
        ...
        // It's a group.
        const groupExtensions = CoreMimetypeUtils.getGroupMimeInfo(
            filetype,
            "extensions"
        );
        const groupMimetypes = CoreMimetypeUtils.getGroupMimeInfo(
            filetype,
            "mimetypes"
        );
        ...
        if (groupExtensions && groupExtensions.length > 0) {
          ...
          typesInfo.push({
            name:
                mimetype && CoreMimetypeUtils.getMimetypeDescription(mimetype),
            extlist: filetype,
          });
  ...
  async uploadOrReuploadFiles(
    ...
    for (let i = 1; i < files.length; i++) {
      const file = files[i];
      promises.push(
          this.uploadOrReuploadFile(file, itemId, component, componentId, siteId)
      );
    }
