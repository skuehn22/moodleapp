...
export class CoreWSProvider {
  ...
  createFakeWSError(
    ...
    translateParams?: { [name: string]: string }
  ): CoreError {
    if (needsTranslate) {
   ...
   */
  throwOnFailedStatus(
    response: CoreStatusWithWarningsWSResponse,
    ...
    try {
      ...
      if (addExtension) {
        ...
        if (
          ...
          if (mimetype) {
            const remoteExtension = CoreMimetypeUtils.getExtension(
              mimetype,
              url
            );
   ...
   */
  protected getPromiseHttp<T = unknown>(
    method: string,
    url: string,
  ...
  getRequestTimeout(): number {
    return CoreApp.isNetworkAccessLimited()
      ? CoreConstants.WS_TIMEOUT
      : CoreConstants.WS_TIMEOUT_WIFI;
  }
   ...
   * @return Promise resolved with the response data in success and rejected with CoreAjaxError.
   */
  protected performAjax<T = unknown>(
    method: string,
    data: Record<string, unknown>,
    preSets: CoreWSAjaxPreSets
  ): Promise<T> {
  ...
  async performPost<T = unknown>(
    ...
    if (
      preSets.typeExpected == "number" ||
      preSets.typeExpected == "boolean" ||
      preSets.typeExpected == "string"
    ) {
    ...
    for (
      ...
      promises.push(
        this.performSinglePost(method, siteUrl, limitedData, preSets, options)
      );
    }
    ...
    if (preSets.splitRequest.combineCallback) {
      return <T>(
        results.reduce(preSets.splitRequest.combineCallback, firstResult)
      );
    }
    ...
    return <T>(
      results.reduce(
        (previous: T, current: T) =>
          this.combineObjectsArrays<T>(previous, current),
        firstResult
      )
    );
  }
  ...
  protected combineObjectsArrays<T>(object1: T, object2: T): T {
    for (const name in object2) {
      ...
      if (Array.isArray(value)) {
        (object1 as Record<string, unknown>)[name] = (
          object1[name] as typeof value
        ).concat(value);
      }
    }
  ...
  protected performSinglePost<T>(
    ...
    const promise = Http.post(requestUrl, ajaxData, options)
      .pipe(timeout(this.getRequestTimeout()))
      .toPromise();
    ...
    return promise.then(
        ...
        if (!data) {
          ...
          if (typeof data == "string") {
            if (preSets.typeExpected == "number") {
              ...
              if (isNaN(data)) {
                this.logger.warn(
                  `Response expected type "${preSets.typeExpected}" cannot be parsed to number`
                );

                throw new CoreError(
                  Translate.instant("core.errorinvalidresponse")
                );
              }
              ...
              if (data === "true") {
                ...
                this.logger.warn(
                  `Response expected type "${preSets.typeExpected}" is not true or false`
                );
              ...
              this.logger.warn(
                'Response of type "' +
                  typeof data +
                  `" received, expecting "${preSets.typeExpected}"`
              );
      ...
      (error) => {
        ...
        if (error.status == 429) {
          const retryPromise = this.addToRetryQueue<T>(
            method,
            siteUrl,
            ajaxData,
            preSets
          ...
          if (this.retryTimeout == 0) {
            this.retryTimeout =
              parseInt(error.headers.get("Retry-After"), 10) || 5;
            this.logger.warn(
  ...
  syncCall<T = unknown>(method: string, data: any, preSets: CoreWSPreSets): T {
    ...
    if (data == null) {
      ...
      throw new CoreError(
        Translate.instant("core.unicodenotsupportedcleanerror")
      );
    }
    ...
    if (
      ...
      typeof data.debuginfo != "undefined"
    ) {
      throw new CoreWSError(data);
    }
  ...
  async uploadFile(
    ...
    const uploadUrl =
      preSets.siteUrl + "/webservice/upload.php?XDEBUG_SESSION_START=18419";
    const transfer = FileTransfer.create();
   ...
   * @return Promise resolved with the response.
   */
  async sendHTTPRequest<T = unknown>(
    url: string,
    ...
    if (CoreApp.isMobile()) {
      ...
      return NativeHttp.sendRequest(url, options).then(
        (response) => new CoreNativeToAngularHttpResponse(response)
      );
    } else {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
