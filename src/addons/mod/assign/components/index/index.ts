...
export class AddonModAssignIndexComponent
  extends CoreCourseModuleMainActivityComponent
  implements OnInit, OnDestroy
{
  @ViewChild(AddonModAssignSubmissionComponent)
  submissionComponent?: AddonModAssignSubmissionComponent;
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.startedObserver = CoreEvents.on(
      AddonModAssignProvider.STARTED_EVENT,
      (data) => {
  ...
  protected async fetchContent(
    ...
    // Get assignment data.
    this.assign = await AddonModAssign.getAssignment(
      this.courseId,
      this.module.id
    );
    ...
    this.hasOffline = await AddonModAssignOffline.hasAssignOfflineData(
      this.assign.id
    );

    // Get assignment submissions.
    const submissions = await AddonModAssign.getSubmissions(this.assign.id, {
      cmId: this.module.id,
    });
    ...
    if (submissions.canviewsubmissions) {
      ...
      this.groupInfo = await CoreGroups.getActivityGroupInfo(
        this.assign.cmid,
        false
      );

      await this.setGroup(
        CoreGroups.validateGroupId(this.group, this.groupInfo)
      );
  ...
  protected async logActivity(): Promise<void> {
    ...
    if (this.canViewAllSubmissions) {
      ...
      CoreUtils.ignoreErrors(
        AddonModAssign.logGradingView(this.assign.id, this.assign.name)
      );
    } else if (this.canViewOwnSubmission) {
      // User can only see their own submission, log view the user submission.
      CoreUtils.ignoreErrors(
        AddonModAssign.logSubmissionView(this.assign.id, this.assign.name)
      );
    }
  }
  ...
  async setGroup(groupId = 0): Promise<void> {
    ...
    const submissionStatus = await AddonModAssign.getSubmissionStatus(
      this.assign.id,
      {
        groupId: this.group,
        cmId: this.module.id,
   ...
   */
  goToSubmissionList(
    status?: AddonModAssignListFilterName,
    ...
    CoreNavigator.navigateToSitePath(
      AddonModAssignModuleHandlerService.PAGE_NAME +
        `/${this.courseId}/${this.module.id}/submission`,
      {
        params,
      }
    );
  ...
  protected async invalidateContent(): Promise<void> {
    ...
    promises.push(
      this.submissionComponent?.invalidateAndRefresh(true) || Promise.resolve()
    );
    ...
    if (this.assign) {
      ...
      if (this.canViewAllSubmissions) {
        promises.push(
          AddonModAssign.invalidateSubmissionStatusData(
            this.assign.id,
            undefined,
            this.group
          )
  ...
  protected isRefreshSyncNeeded(
    syncEventData: AddonModAssignAutoSyncData
  ): boolean {
