...
export class AddonModChoiceIndexComponent
  ...
  implements OnInit
{
  ...
  protected isRefreshSyncNeeded(
    syncEventData: AddonModChoiceAutoSyncData
  ): boolean {
    if (
      this.choice &&
      syncEventData.choiceId == this.choice.id &&
      syncEventData.userId == this.userId
    ) {
  ...
  protected async fetchContent(
    ...
    if (sync) {
      ...
      if (updated) {
        ...
        this.choice = await AddonModChoice.getChoice(
          this.courseId,
          this.module.id
        );
      }
    ...
    this.choiceNotOpenYet =
      !!this.choice.timeopen && this.choice.timeopen > this.now;
    this.choiceClosed =
      !!this.choice.timeclose && this.choice.timeclose <= this.now;
  ...
  protected async fetchOptions(choice: AddonModChoiceChoice): Promise<void> {
    let options = await AddonModChoice.getOptions(choice.id, {
      cmId: this.module.id,
    });
  ...
  protected async fetchResults(choice: AddonModChoiceChoice): Promise<void> {
    ...
    this.canSeeResults =
      hasVotes ||
      AddonModChoice.canStudentSeeResults(choice, this.hasAnsweredOnline);
  }
  ...
  protected isChoiceOpen(choice: AddonModChoiceChoice): boolean {
    return (
      (!choice.timeopen || choice.timeopen <= this.now) &&
      (!choice.timeclose || choice.timeclose > this.now)
    );
  }
  ...
  async save(): Promise<void> {
    ...
    try {
      const online = await AddonModChoice.submitResponse(
        choice.id,
        choice.name,
        this.courseId,
        responses
  ...
  async delete(): Promise<void> {
    ...
    try {
      await AddonModChoice.deleteResponses(
        this.choice!.id,
        this.choice!.name,
        this.courseId
      );
  ...
  protected async dataUpdated(online: boolean): Promise<void> {
    ...
    try {
      ...
      await AddonModChoiceSync.prefetchAfterUpdate(
        AddonModChoicePrefetchHandler.instance,
        this.module,
        this.courseId
      );
