...
export const CALENDAR_SITE_SCHEMA: CoreSiteSchema = {
  ...
  async migrate(
    ...
    if (oldVersion < 4) {
      ...
      const key =
        AddonCalendarProvider.DEFAULT_NOTIFICATION_TIME_SETTING + "#" + siteId;
      const defaultTime = await CoreUtils.ignoreErrors(
        CoreConfig.get(key, null)
      );
      ...
      const records = await db.getAllRecords<AddonCalendarReminderDBRecord>(
        oldTable
      );
      ...
      await Promise.all(
        records.map(async (record) => {
          ...
          if (!events[record.eventid]) {
            try {
              events[record.eventid] = await db.getRecord(EVENTS_TABLE, {
                id: record.eventid,
              });
            } catch {
          ...
          if (!record.time || record.time === -1) {
            ...
            record.time =
              events[record.eventid].timestart -
              Math.floor(record.time / 60) * 60;
          }
